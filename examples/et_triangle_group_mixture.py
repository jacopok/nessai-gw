...
from nessai_gw.group_mixture import ET_TRIANGLE_PARAMETERS, ETTriangleGroupAction
...
def main() -> None:
    ...
    tp = {k: torch.as_tensor(post[k]) for k in keys}

    lam, beta, _ = action._to_frame(tp["ra"], tp["dec"], tp["psi"])
    lam = np.mod(lam.numpy(), 2 * np.pi)
    beta = beta.numpy()

    # Fold: keep, for each sample, the unique image in the fundamental domain.
    folded = {k: np.array(post[k]) for k in keys}
    ...
    lam_f, beta_f, _ = action._to_frame(fp["ra"], fp["dec"], fp["psi"])
